  ...
  for (const doc of snapshot.docs) {
    ...
    if (daysLeft === 2 && lastNotified['2days'] !== todayStr) {
      ...
      const sent = await sendNotification(
        ...
        message,
        'Birthday Reminder',
        doc.id // 🟢 CHANGE: pass unique id
      );
      if (sent) {
        await doc.ref.update({[`lastNotified.2days`]: todayStr});
      }
    }
    ...
    if (daysLeft === 1 && lastNotified['1day'] !== todayStr) {
      ...
      if (sent) {
        await doc.ref.update({[`lastNotified.1day`]: todayStr});
      }
    }
    ...
    try {
      ...
      for (const holiday of holidays) {
        ...
        const sent = await sendNotification(
          ...
          message, 
          'Today is holiday',
          holiday.date.iso // 🟢 CHANGE: unique per holiday
        );
        if (sent) {
          await doc.ref.update({ lastHolidayNotified: todayStr });
        }
